...
class AMapView extends Component {
    ...
    _onMapLoaded() {
        const {region, initialRegion, onMapLoaded} = this.props;
        if (region) {
            this.map.setNativeProps({
                region
            })
        ...
        this.setState({isReady: true}, () => {
            if (onMapLoaded) onMapLoaded(); // for user override
        });
    ...
    _onChange(event) {
        ...
        } else if (this.props.onRegionChange) {
            this.props.onRegionChange(event.nativeEvent.region);
        }
    }
    ...
    _uiManagerCommand(name) {
        return NativeModules.UIManager[getAMapComponent(this.props.provider)].Commands[name]
    }
